...
export function Offer() {
  return (
    <FramedSection id="offer" frameClassName="bg-cream">
      <Stirrup
        strokeWidth={0.5}
        className="pointer-events-none absolute -right-20 -bottom-16 hidden size-[30rem] text-coral opacity-[0.06] lg:block"
      />
        ...
        <div className="mt-14 grid gap-5 md:grid-cols-3">
          {TIERS.map((tier, i) => (
            <Reveal key={tier.name} delay={i * 100}>
              <article className="group flex h-full flex-col rounded-3xl border border-line bg-paper-2 p-7 transition-all duration-300 hover:-translate-y-1 hover:border-teal/40 hover:shadow-[0_28px_60px_-40px_rgba(61,38,28,0.6)]">
                <div className="flex items-center justify-between gap-3">
                  <span className="label-xs text-teal">{tier.tag}</span>
                  {/* was 0.58rem in --coral, which is 2.9:1 on this cream — the
                      smallest type on the page in the lowest-contrast colour */}
                  <span className="label-xs rounded-full border border-coral/40 bg-coral/[0.07] px-2.5 py-0.5 text-coral-ink">
                    By quote
                  </span>
                </div>
                ...
                <span className="mt-6 h-px w-10 bg-coral transition-all duration-300 group-hover:w-20" />
              </article>
            </Reveal>
          ))}
        </div>
